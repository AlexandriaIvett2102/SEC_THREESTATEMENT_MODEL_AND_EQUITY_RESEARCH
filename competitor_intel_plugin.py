# Created plug in for competitor analysis
...
# Peers maxed for computing power and ease of reading the resulting workbook
MAX_PEERS = 10

# Function that orchestrates the entire research workbook generation process for a given ticker. It performs the following steps:
# 1. Resolves the CIK and company name from the ticker.
# 2. Fetches the company's SEC submissions and profile.
# 3. Retrieves the latest and prior 10-K filings, extracts the Business and Risk Factors sections, and identifies new risk statements.
# 4. Mines competitor names from the Business section and cross-checks against the ticker map and SIC peers to build a peer group.
# 5. Computes market share and revenue trends across the peer group.
# 6. Classifies product type and end markets.
# 7. Builds the Excel workbook with the three-statement model and attaches the Competitive Landscape, Industry Factors, and Signal Summary sheets.
...
# On click run pipeline and display link to resulting workbook, along with a button to open it directly
